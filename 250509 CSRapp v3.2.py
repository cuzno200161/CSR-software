...
class CSRApp:
    def __init__(self, root):
        ...
        # Prefer 'vista' on Windows, 'clam' on other platforms if available, else default
        if 'vista' in available_themes and self.root.tk.call('tk', 'windowingsystem') == 'win32':
            self.style.theme_use('vista')
        elif 'clam' in available_themes:
            self.style.theme_use('clam')
        elif 'aqua' in available_themes and self.root.tk.call('tk', 'windowingsystem') == 'aqua': # macOS
            self.style.theme_use('aqua')


        # Define custom fonts
        base_font_family = "Segoe UI" if self.root.tk.call('tk', 'windowingsystem') == 'win32' else "Helvetica" # Basic cross-platform choice
        
        self.default_font = tkFont.nametofont("TkDefaultFont")
        self.default_font.configure(family=base_font_family, size=10)
        self.root.option_add("*Font", self.default_font)

        # Attempt to use Semibold if available, otherwise bold
        try:
            # Check if "Family Semibold" exists
            tkFont.Font(family=f"{base_font_family} Semibold", size=10).actual()
            semibold_family = f"{base_font_family} Semibold"
            bold_weight = tkFont.NORMAL # if Semibold family includes the weight
        except tk.TclError:
        ...
        self.style.configure("TButton", padding=(8, 6), relief="flat", font=self.button_font) # Increased padding
        self.style.map("TButton",
            foreground=[('pressed', 'black'), ('active', '#005A9E')], # Darker blue on active
            background=[('pressed', '!disabled', '#E0E0E0'), ('active', '#E5F1FB')] # Lighter active background
        )
        self.style.configure("TLabel", font=self.label_font, padding=(0, 2, 0, 2))
        self.style.configure("TEntry", font=self.entry_font, padding=4) # Increased padding
        ...
        self.style.configure("TLabelframe.Label", font=self.title_font, padding=(0, 0, 0, 5)) # Padding below label
        self.style.configure("TNotebook.Tab", font=(semibold_family, 10, bold_weight if semibold_family == base_font_family else tkFont.NORMAL), padding=[12, 6]) # Increased tab padding
    ...
    def create_csr_integration_tab(self):
        ...
        ttk.Label(left_frame, text="CSR Function Model", font=self.title_font).pack(anchor='w', padx=5, pady=(10,2))
        eqn_text_frame = ttk.Frame(left_frame, style="App.TFrame")
        eqn_text_frame.pack(fill='x', expand=False, padx=5, pady=(0,5))
        
        self.equation_text = tk.Text(eqn_text_frame, height=8, wrap=tk.WORD, state='disabled',
        ...
        self.equation_text.pack(side=tk.LEFT, fill='x', expand=True)


        results_frame = ttk.LabelFrame(left_frame, text="Key Results", padding=(10,5,10,10))
        results_frame.pack(fill='x', pady=10, padx=5)
        
        ttk.Label(results_frame, text="Extremum (Original Scale):", font=(self.label_font.cget("family"), self.label_font.cget("size"), tkFont.BOLD)).grid(row=0, column=0, columnspan=2, sticky='w', pady=(0,8))
        
        ttk.Label(results_frame, text="Factors:").grid(row=1, column=0, sticky='w', padx=(0,10))
        self.factors_text = tk.Text(results_frame, height=1, wrap=tk.NONE, state='disabled', font=self.text_widget_font, relief=tk.SOLID, borderwidth=1, padx=5)
        ...
        self.value_text = tk.Text(results_frame, height=1, wrap=tk.NONE, state='disabled', font=self.text_widget_font, relief=tk.SOLID, borderwidth=1, padx=5)
        ...
        self.r2_text = tk.Text(results_frame, height=1, wrap=tk.NONE, state='disabled', font=self.text_widget_font, relief=tk.SOLID, borderwidth=1, padx=5)
        self.r2_text.grid(row=3, column=1, sticky='ew', pady=3)
        results_frame.columnconfigure(1, weight=1)


        # === Center Panel Contents ===
        table_container = ttk.Frame(center_frame, style="App.TFrame")
        table_container.pack(fill='both', expand=True, pady=(0,5)) # Adjusted padding
        ...
        right_paned.add(plot1_frame) # Removed weight here
    ...
    def create_coefficient_analysis_tab(self):
        coeff_tab_frame = ttk.Frame(self.tab2, padding=10, style="App.TFrame")
        coeff_tab_frame.pack(expand=True, fill='both')

        controls_frame = ttk.LabelFrame(coeff_tab_frame, text="Analysis Controls", padding=10)
        ...
        charts_frame = ttk.Frame(coeff_tab_frame, style="App.TFrame")
        ...
        for i, title_text in enumerate(chart_titles):
            row, col = divmod(i, 2)
            ...
            fig.subplots_adjust(left=0.05, right=0.50, top=0.92, bottom=0.08)
        ...
        self.update_coefficient_pie_charts(initial_load=True)
    ...
    def update_alpha_value(self, value_str):
        try:
    ...
    def run_fitting(self):
        ...
        try:
            ...
            try:
                ...
                if model.n_iter_ == 1000:  # scikit-learn's default max_iter
                ...
                messagebox.showerror("Fitting Error", f"Regression failed to converge:\n{str(e)}")
                self.coefficients = None
                self.extremum_point = None
                self.clear_results_and_plots()
                return
            ...
            eqn_str_with_defs = self.generate_equation_with_definitions(self.coefficients, self.bits_array, n_factors)
            self.update_results_display(eqn_str_with_defs, train_r2)
            self.update_table_view()
    ...
    def update_results_display(self, eqn_str_with_defs, train_r2):
        self.equation_text.config(state='normal')
        self.equation_text.delete(1.0, tk.END)
        self.equation_text.insert(tk.END, eqn_str_with_defs)
        self.equation_text.config(state='disabled')
        ...
        if self.extremum_point and 'x' in self.extremum_point and self.extremum_point['x'] is not None and len(self.extremum_point['x']) > 0:
            extremum_x_in_fitting_scale = self.extremum_point['x']
            extremum_x_original_scale = self._unnormalize_point(extremum_x_in_fitting_scale)
    ...
    def generate_equation_with_definitions(self, beta, bits_array, n_factors):
        ...
        if n_factors > 0 and self.factor_cols: # Ensure factor_cols is populated
            for i in range(n_factors):
                ...
                definitions.append(f"  {short_name} = {original_long_name}")
        elif n_factors > 0 : # Fallback if self.factor_cols not ready but n_factors known
             for i in range(n_factors):
                definitions.append(f"  f{i+1} = Factor {i+1} (Original Name Not Available)")


        return final_equation_str + "\n".join(definitions)
    ...
    def update_coefficient_pie_charts(self, initial_load=False): # Simplified for brevity, refer to previous for full logic
        for i in range(4):
            if i not in self.pie_axes: return
            self.pie_axes[i].cla(); self.pie_figures[i].subplots_adjust(left=0.05, right=0.50, top=0.92, bottom=0.08); self.pie_figures[i].patch.set_facecolor('#F0F0F0'); self.pie_axes[i].set_facecolor('#FFFFFF')
        ...
        if initial_load or self.coefficients is None or self.bits_array is None or self.df is None or not self.factor_cols:
            if not initial_load and (self.coefficients is None or self.bits_array is None or self.df is None or not self.factor_cols) : messagebox.showinfo("Coefficient Analysis", "Please load data and run fitting first.")
            for i in range(4): self.pie_axes[i].text(0.5, 0.5, placeholder_text, ha='center', va='center', fontsize=9, color='gray'); self.pie_canvas[i].draw()
            return
        ...
        if x_eval_original_scale_for_contrib is None:
             ...
             for i in range(4): self.pie_axes[i].text(0.5, 0.5, "Error: No eval point.", ha='center', va='center', fontsize=9, color='red'); self.pie_canvas[i].draw()
             return
        
        n_factors_model = self.bits_array.shape[1]
        if len(x_eval_original_scale_for_contrib) != n_factors_model :
            messagebox.showerror("Dimension Mismatch", f"Eval point factors ({len(x_eval_original_scale_for_contrib)}) vs Model factors ({n_factors_model}). Pie charts may be incorrect. Re-run fitting."); # Fall through with warning

        term_contributions_map = {'constant': 0.0, 'linear_total': 0.0, 'quadratic_total': 0.0, 'interaction_total': 0.0, 'linear_factors': {}, 'quadratic_factors': {}, 'interaction_factors': {}}
        for coef_val, bits_def in zip(self.coefficients, self.bits_array):
            ...
            sum_of_powers_in_bits, unique_powers_in_bits = np.sum(bits_def), set(bits_def)
            if sum_of_powers_in_bits == 0: term_contributions_map['constant'] += contribution_val
            elif sum_of_powers_in_bits == 1 and unique_powers_in_bits.issubset({0, 1}):
                idx = np.where(bits_def == 1)[0][0]; name = self.col_name_mapping.get(f"factor{idx+1}", f"f{idx+1}?");
                term_contributions_map['linear_total'] += contribution_val; term_contributions_map['linear_factors'][name] = term_contributions_map['linear_factors'].get(name, 0) + contribution_val
            elif sum_of_powers_in_bits == 2 and 2 in unique_powers_in_bits:
                idx = np.where(bits_def == 2)[0][0]; name = self.col_name_mapping.get(f"factor{idx+1}", f"f{idx+1}?"); d_name = f"{name}\u00B2";
                term_contributions_map['quadratic_total'] += contribution_val; term_contributions_map['quadratic_factors'][d_name] = term_contributions_map['quadratic_factors'].get(d_name, 0) + contribution_val
            elif sum_of_powers_in_bits == 2 and unique_powers_in_bits.issubset({0, 1}):
                idxs = np.where(bits_def == 1)[0]
                if len(idxs) == 2: name1,name2 = self.col_name_mapping.get(f"factor{idxs[0]+1}", f"f{idxs[0]+1}?"), self.col_name_mapping.get(f"factor{idxs[1]+1}", f"f{idxs[1]+1}?"); d_name = " \u00D7 ".join(sorted([name1, name2]));
                term_contributions_map['interaction_total'] += contribution_val; term_contributions_map['interaction_factors'][d_name] = term_contributions_map['interaction_factors'].get(d_name, 0) + contribution_val
        
        def plot_custom_pie(ax, data_values_map, chart_title_suffix, canvas_to_draw): # Simplified for brevity
            ax.cla(); fig_parent = ax.get_figure(); fig_parent.subplots_adjust(left=0.05, right=0.50, top=0.92, bottom=0.08); fig_parent.patch.set_facecolor('#F0F0F0'); ax.set_facecolor('#FFFFFF')
            for spine in ax.spines.values(): spine.set_visible(False);
            ax.set_xticks([]); ax.set_yticks([])
            if not data_values_map: ax.text(0.5, 0.5, f"No significant {chart_title_suffix.lower()} contributions", ha='center', va='center', fontsize=8, wrap=True, color='gray'); canvas_to_draw.draw(); return
            labels_raw, sizes_raw = list(data_values_map.keys()), [abs(s) for s in data_values_map.values()]
            valid_slices = [{'label': l, 'size': s, 'original_value': ov} for l, s, ov in zip(labels_raw, sizes_raw, data_values_map.values()) if s > 1e-7]
            if not valid_slices: ax.text(0.5, 0.5, f"Contributions for {chart_title_suffix.lower()} are negligible.", ha='center', va='center', fontsize=8, wrap=True, color='gray'); canvas_to_draw.draw(); return
            final_labels, final_sizes_abs, final_original_values = [i['label'] for i in valid_slices], np.array([i['size'] for i in valid_slices]), [i['original_value'] for i in valid_slices]
            total_abs_size = np.sum(final_sizes_abs)
            if total_abs_size <= 1e-7: ax.text(0.5, 0.5, f"Total for {chart_title_suffix.lower()} is negligible.", ha='center', va='center', fontsize=8, wrap=True, color='gray'); canvas_to_draw.draw(); return
            legend_labels = [f"{l}: {ov:+.2e} ({s/total_abs_size*100:.1f}%)" if abs(ov) > 1e4 or (abs(ov) < 1e-2 and abs(ov) > 0) else f"{l}: {ov:+.3f} ({s/total_abs_size*100:.1f}%)" for l,s,ov in zip(final_labels, final_sizes_abs, final_original_values)]
            
            # Color selection: using 'Set2' for <8, 'tab10' for <10, 'tab20' for <20
            num_colors = len(final_labels)
            if num_colors <= 8: colors = plt.cm.get_cmap('Set2', num_colors).colors
            elif num_colors <= 10: colors = plt.cm.get_cmap('tab10', num_colors).colors
            elif num_colors <=20 : colors = plt.cm.get_cmap('tab20b', num_colors).colors # tab20b has better distinct light/dark pairs
            else: colors = plt.cm.get_cmap('viridis', num_colors).colors # Fallback

            wedges, _ = ax.pie(final_sizes_abs, colors=colors, startangle=120, wedgeprops={'edgecolor': '#404040', 'linewidth': 0.7})
            ax.axis('equal'); ax.legend(wedges, legend_labels, title="Breakdown (Value & % of Total Mag.)", loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=8, title_fontsize=9, labelspacing=0.8, borderpad=0.8, frameon=True, facecolor='#f8f8f8', edgecolor='gray')
            canvas_to_draw.draw()

        overall_term_types_data = {'Constant': term_contributions_map['constant'], 'Linear': term_contributions_map['linear_total'], 'Quadratic': term_contributions_map['quadratic_total'], 'Interaction': term_contributions_map['interaction_total']}
        plot_custom_pie(self.pie_axes[0], overall_term_types_data, "Overall Term Type", self.pie_canvas[0])
        plot_custom_pie(self.pie_axes[1], term_contributions_map['linear_factors'], "Linear Factor", self.pie_canvas[1])
        plot_custom_pie(self.pie_axes[2], term_contributions_map['quadratic_factors'], "Quadratic Factor", self.pie_canvas[2])
        plot_custom_pie(self.pie_axes[3], term_contributions_map['interaction_factors'], "Interaction Factor", self.pie_canvas[3])
