...
class CSRApp:
    ...
    def create_csr_integration_tab(self):
        ...
        self.equation_text = tk.Text(eqn_text_frame, height=6, wrap=tk.WORD, state='disabled',
        ...
        self.factor_definitions_text = tk.Text(factor_def_text_frame, height=4, wrap=tk.WORD, state='disabled',
                                               font=self.text_widget_font, relief=tk.SOLID, borderwidth=1, padx=8, pady=5,spacing1=2, spacing2=2, spacing3=2)
    ...
    def create_coefficient_analysis_tab(self):
        ...
        # Definitions frame - always visible
    ...
    def update_results_display(self, equation_str, function_defs_str, factor_defs_str, train_r2):
        ...
        # Update Function Definitions Text
        # (Note: This app doesn't currently generate separate function definitions,
        # but the structure is here if needed in the future. For now, it might be empty or contain a placeholder)
        # self.function_definitions_text.config(state='normal')
        # self.function_definitions_text.delete(1.0, tk.END)
        # self.function_definitions_text.insert(tk.END, function_defs_str)
        # self.function_definitions_text.config(state='disabled')
    ...
    def update_coefficient_pie_charts(self, initial_load=False):
        # Update factor definitions for Coefficient Analysis tab
        factor_definitions = []

        if hasattr(self, 'factor_cols') and self.factor_cols and hasattr(self, 'col_name_mapping'):
            for i, factor in enumerate(self.factor_cols):
                short_name = f"f{i+1}"
                original_long_name = self.col_name_mapping.get(factor, f"Factor {i+1} (Name Unknown)")
                factor_definitions.append(f"{short_name} = {original_long_name}")
        else: # Fallback if factor_cols or col_name_mapping not ready
             if hasattr(self, 'df') and self.df is not None:
                  factor_columns_in_df = [col for col in self.df.columns if col.startswith('factor')]
                  for i, factor in enumerate(factor_columns_in_df):
                       short_name = f"f{i+1}"
                       # Cannot get original name without mapping
                       factor_definitions.append(f"f{i+1} = Factor {i+1} (Name Unknown)")


        # Update definitions text widget in Coefficient Analysis tab
        if hasattr(self, 'definitions_text'):
            ...
            if factor_definitions:
                ...
                self.definitions_text.insert(tk.END, "No factor definitions available")
            self.definitions_text.config(state=tk.DISABLED)
        ...
        if initial_load or self.coefficients is None or self.bits_array is None or self.df is None or not self.factor_cols:
            if not initial_load and (self.coefficients is None or self.bits_array is None or self.df is None or not self.factor_cols):
                # Only show error if not initial load
                 pass # Message box already handled in run_fitting


            for i in range(4):
                # Clear previous plot elements but keep title/frame
                self.pie_axes[i].cla()
                # Redraw the placeholder text
                self.pie_axes[i].text(0.5, 0.5, placeholder_text,
                                     ha='center', va='center',
        ...
        if x_eval_original_scale_for_contrib is None:
            ...
            for i in range(4):
                self.pie_axes[i].cla()
                self.pie_axes[i].text(0.5, 0.5, "Error: No eval point.",
                                     ha='center', va='center',
        ...
        if len(x_eval_original_scale_for_contrib) != n_factors_model:
            # This warning is less critical for pie charts than for other calculations,
            # but still indicates a potential issue.
             messagebox.showwarning("Dimension Mismatch",
                               f"Eval point factors ({len(x_eval_original_scale_for_contrib)}) vs Model factors ({n_factors_model}). Pie charts may be misleading.")
        ...
        for coef_val, bits_def in zip(self.coefficients, self.bits_array):
            ...
            if sum_of_powers_in_bits == 0:
                ...
                name = f"f{idx+1}"  # Use short names (f1, f2, etc.) in pie charts
                ...
                name = f"f{idx+1}"
                d_name = f"{name}\u00B2"  # Quadratic term notation
                ...
                if len(idxs) == 2:
                    name1 = f"f{idxs[0]+1}"
                    name2 = f"f{idxs[1]+1}"
                    d_name = " \u00D7 ".join(sorted([name1, name2]))  # Interaction term notation
        ...
        def plot_custom_pie(ax, data_values_map, chart_title_suffix, canvas_to_draw):
            ax.cla()
            fig_parent = ax.get_figure()
            fig_parent.subplots_adjust(left=0.05, right=0.50, top=0.92, bottom=0.08)
            ...
            labels_raw = list(data_values_map.keys())
            sizes_raw = [abs(s) for s in data_values_map.values()]

            valid_slices = []
            for l, s, ov in zip(labels_raw, sizes_raw, data_values_map.values()):
                if s > 1e-7:
                    valid_slices.append({
                        'label': l,
                        'size': s,
            ...
            formatted_values = []
            for val in final_original_values:
                if abs(val) > 1e4 or (abs(val) < 1e-2 and abs(val) > 0):
                    formatted_values.append(f"{val:+.2e}")
                else:
                    formatted_values.append(f"{val:+.3f}")

            # Calculate percentages
            percentages = [s/total_abs_size*100 for s in final_sizes_abs]

            # Create legend labels
            legend_labels = [
                f"{l}: {fv} ({p:.1f}%)"
                for l, fv, p in zip(final_labels, formatted_values, percentages)
            ]

            # Color selection
            num_colors = len(final_labels)
            if num_colors <= 8:
                colors = plt.cm.get_cmap('Set2', num_colors).colors
            elif num_colors <= 10:
                colors = plt.cm.get_cmap('tab10', num_colors).colors
            elif num_colors <= 20:
                colors = plt.cm.get_cmap('tab20b', num_colors).colors
            else:
                colors = plt.cm.get_cmap('viridis', num_colors).colors
            ...
            ax.legend(
                ...
                title=f"{chart_title_suffix} Terms (Value & % of Total Mag.)",
                loc="center left",
